...
class IPv6AddressSpec extends Specification with DataTables { def is =
  ...
  "The IPv6 regular expression should only work with valid ip6 addresses" ! {
    val fd = ListBuffer[String]()
    addresses foreach {
      case (isValid, candidate) => {
        if(TestIPAddressParser.parseAddress(candidate) != isValid) {
          val exp = if (isValid) "" else "in"
          val opp = if (isValid) "in" else ""
          fd += "The address '%s' should be %svalid but it's %svalid".format(candidate, exp, opp)
        }
      }
    }
    println("there are %s failures: " format fd.length)
    println(fd mkString "\n")
    fd.toList must beEmpty
  }



  val addresses = List( (false,""),
                        (true,"::1"),
                        (true,"::"),
